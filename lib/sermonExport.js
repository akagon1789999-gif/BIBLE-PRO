...
function buildTranscriptText({ transcript, verses, startedAt }) {
  const lines = [];
  lines.push("SERMON EXPORT — Sofer 2.0");
  ...
  if (verses.length) {
    ...
    for (const v of verses) {
      const refLine = v.translation ? `${v.ref} (${v.translation})` : v.ref;
      lines.push(`${refLine}  —  ${formatTimestamp(v.timestamp)}`);
...
function buildTranscriptPdf(doc, { transcript, verses, startedAt }) {
  ...
  if (verses.length) {
    ...
    for (const v of verses) {
      const refLine = v.translation ? `${v.ref} (${v.translation})` : v.ref;
      doc.fontSize(12).font("Helvetica-Bold").fillColor("#000000").text(refLine);
