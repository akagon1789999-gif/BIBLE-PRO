...
// Fetches every verse in a chapter, for the Bible Browser panel. Distinct
// from fetchVerses/fetchVerseRange, which stitch verses into one string for
// the projection use case — here each verse stays separate for a scrollable
// verse-by-verse list.
async function fetchChapter(bookId, chapter, translation = "KJV") {
  const res = await fetch(`${BASE_URL}/get-text/${translation}/${bookId}/${chapter}/`);
  if (!res.ok) throw new Error(`Bolls API error ${res.status}`);
  const data = await res.json();
  return data.map((v) => ({ verse: v.verse, text: stripHtml(v.text) }));
}
