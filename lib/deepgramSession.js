...
// The SDK's own default is a hard 10s — too tight for slower/congested
// networks, where the handshake can genuinely take longer without being
// truly blocked (seen in practice as a bare "TIMEOUT" error with no other
// info). 30s gives real connections more room without hanging forever on
// a truly dead one.
const DEEPGRAM_CONNECT_TIMEOUT_SECONDS = Number(process.env.DEEPGRAM_CONNECT_TIMEOUT_SECONDS) || 30;
...
async function startSession({ onFinal, onInterim, onError, onClose }) {
  ...
  const connection = await client.listen.v1.connect({
    ...
    connectionTimeoutInSeconds: DEEPGRAM_CONNECT_TIMEOUT_SECONDS,
  });
